...
def get_category(weights, image):
    imagenet_categories = np.asarray(weights.meta["categories"])
    image_cat = po.to_numpy(torch.nn.functional.softmax(tv_model(norm(image))).squeeze())
    return imagenet_categories[image_cat > 0.1]

def main(seed=None, lr=0.01, max_iter=1000, save_path="metamer.pt", device="cpu",
         scheduler=None, optimizer="Adam", loss="norm_mse"):
    ...
    model = po.models.FeatureExtractorModel(tv_model, "layer3", norm)
    ...
    orig_image_category = get_category(weights, met.image)
    met_image_category = get_category(weights, met.metamer)
    pearson_corr = torch.corrcoef(torch.cat([model(met.metamer), model(met.image)], 0))[0, 1].item()

    fig.text(0.5, 1.5, f"Image category: {orig_image_cateogry}\nMetamer category:{met_image_category}\nPearson R: {pearson_corr}",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss", default="norm_mse")
